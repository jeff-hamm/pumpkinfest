...
function doOptions(e) {
  // For older Google Apps Script versions, we can't set custom headers
  // Return a simple response that indicates CORS support
  return ContentService
...
function createResponse(data, status = 200) {
  ...
  // For Google Apps Script, we need to use a different approach for CORS
  // We'll add CORS headers in the response data itself and handle it on the client side
  const output = ContentService
    .createTextOutput(JSON.stringify(response))
    .setMimeType(ContentService.MimeType.JSON);
    
  // Try to set headers if supported
  try {
    if (output.setHeaders) {
      output.setHeaders({
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
        'Access-Control-Allow-Headers': 'Content-Type'
    ...
    // Headers not supported in this version
    console.log('Headers not supported, using JSONP fallback');
  }

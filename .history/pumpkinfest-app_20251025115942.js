...
class PumpkinfestRSVP {
    ...
    async handleRSVPSubmit(event) {
        ...
        if (photoFile) {
            try {
                photoUrl = await this.uploadPhotoToDrive(photoFile, guestName);
            } catch (error) {
                console.error('Photo upload failed:', error);
                this.showRSVPLoading(false);
                alert('Photo upload failed, but RSVP will be submitted without photo. Error: ' + error.message);
            }
        }
    ...
    renderRSVPGrid() {
        ...
        filteredRSVPs.forEach(rsvp => {
            ...
            html += `<td class="grid-photo">${this.formatPhoto(rsvp.photoUrl)}</td>`;
            html += `</tr>`;
        });
        
        gridBody.innerHTML = html;
        
        // Update the photo gallery with new photos
    ...
    formatPhoto(photoUrl) {
        if (!photoUrl || photoUrl.trim() === '') {
            return '<span class="no-photo">📷</span>';
        }
        
        // Convert Google Drive URL to direct image URL for thumbnails
        let directUrl = photoUrl;
        if (photoUrl.includes('drive.google.com/file/d/')) {
            const fileId = photoUrl.match(/\/file\/d\/([a-zA-Z0-9_-]+)/)?.[1];
            if (fileId) {
                directUrl = `https://drive.google.com/uc?id=${fileId}`;
            }
        }
        
        return `<a href="${photoUrl}" target="_blank" class="photo-link">
                    <img src="${directUrl}" alt="Pumpkin photo" class="grid-photo-thumb" 
                         onerror="this.parentElement.innerHTML='📷 Photo'">
                </a>`;
    }

    updatePhotoGallery() {
        // Get all RSVPs with photos
        const rsvpsWithPhotos = this.rsvps.filter(rsvp => rsvp.photoUrl && rsvp.photoUrl.trim() !== '');
        
        if (rsvpsWithPhotos.length === 0) {
            return; // Keep existing static gallery if no uploaded photos
        }
        
        const gallery = document.querySelector('.image-gallery');
        
        // Add RSVP photos to the gallery
        rsvpsWithPhotos.forEach(rsvp => {
            // Check if this photo is already in the gallery
            if (gallery.querySelector(`[data-rsvp-photo="${rsvp.id}"]`)) {
                return; // Skip if already added
            }
            
            let directUrl = rsvp.photoUrl;
            if (rsvp.photoUrl.includes('drive.google.com/file/d/')) {
                const fileId = rsvp.photoUrl.match(/\/file\/d\/([a-zA-Z0-9_-]+)/)?.[1];
                if (fileId) {
                    directUrl = `https://drive.google.com/uc?id=${fileId}`;
                }
            }
            ...
            galleryItem.setAttribute('data-rsvp-photo', rsvp.id);
            galleryItem.innerHTML = `
                <img src="${directUrl}" alt="Pumpkin photo by ${this.escapeHtml(rsvp.name)}" class="gallery-img"
                     onerror="this.parentElement.style.display='none'">
                <p class="gallery-caption">Photo by ${this.escapeHtml(rsvp.name)}</p>
            `;

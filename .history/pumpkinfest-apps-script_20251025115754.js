...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'uploadPhoto':
        return createResponse(uploadPhoto(data.filename, data.data, data.mimeType));
...
function getRSVPsFromSheet(sheet) {
  ...
  const photoUrlIndex = findColumnIndex(headers, ['photo url', 'photourl', 'photo', 'image']);
...
function addRSVP(rsvpData) {
  try {
    ...
    if (lastRow === 0) {
      ...
      headers = ['Name', 'Attendance', 'Need Pumpkin', 'Bringing', 'Pumpkin Patch', 'Patch Dates', 'Photo URL', 'Timestamp'];
    ...
    const photoUrlIndex = findColumnIndex(headerMap, ['photo url', 'photourl', 'photo', 'image']);
    ...
    if (photoUrlIndex >= 0) newRow[photoUrlIndex] = rsvpData.photoUrl || '';
